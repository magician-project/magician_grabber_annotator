#!/bin/bash
# Usage: ./countDefectsAndSeverities.sh <dataset_dir> [<dataset_dir> ...] [-- <extra datasetStatistics.py args>]
...
# one or more raw datasets, by invoking datasetStatistics.py from the repo root.
...
# was not captured at the datasetStatistics.py defaults.

if [ -z "$1" ]; then
  echo "Usage: $0 <dataset_dir> [<dataset_dir> ...] [-- <extra datasetStatistics.py args>]"
  echo "Example: $0 /path/to/dataset"
  exit 1
fi

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STATS="$REPO_ROOT/datasetStatistics.py"
